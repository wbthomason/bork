...
multiple types. Operations are run in the order Install, Remove, Update, Search")
        ...
        .subcommand(SubCommand::with_name(constants::UPDATE_CMD)
                        ...
                        .arg(Arg::with_name(constants::PACKAGES)
                                 .help("The list of packages to update. Pass \"all\" or nothing \
...
pub fn get_config_opts(file_path: Option<&str>) -> Vec<Yaml> {
    let file_path = file_path.unwrap_or(if cfg!(target_os = "windows") {
        constants::WINDOWS_CONF_PATH
    } else {
        constants::NIX_CONF_PATH
    });
    ...
    let options = YamlLoader::load_from_str(&config_contents)
                      .ok()
